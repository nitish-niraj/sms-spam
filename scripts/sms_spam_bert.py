...
try:
    ...
    print(f"  (Skipping tight_layout due to: {str(e)[:50]})")
    pass
...
for i in range(0, total_messages, batch_size):
    batch = df['message_clean'].iloc[i:i+batch_size]
    
    for idx, message in enumerate(batch):
        # Cast to string and truncate if too long
        message_str = str(message)[:1000]  # Truncate very long messages
        try:
            tokens = tokenizer.encode(message_str, add_special_tokens=True, truncation=True, max_length=512)
            message_lengths.append(len(tokens))
        except Exception as e:
            print(f"Warning: Error tokenizing message at index {i+idx}: {str(e)[:50]}")
            message_lengths.append(0)
    
    # Progress logging
    if (i + batch_size) % 500 == 0 or (i + batch_size) >= total_messages:
...
training_args = TrainingArguments(
    ...
    per_device_eval_batch_size=16,  # Reduced for CPU
    ...
    use_cpu=True,  # Fixed: Changed from no_cuda to use_cpu
    report_to='none',
    seed=42
)
